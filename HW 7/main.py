...
class Calculator(QMainWindow):
    ...
    def add_digit(self) -> None:
        ...
        if btn.objectName() in Operator.DIGIT_BUTTONS:
    ...
    def calculate(self) -> Optional[str]:
        try:
            result = self.remove_trailing_zeros(
                (Operator.OPERATIONS[self.get_math_sign()](
    ...
    def show_zero_division_error(self) -> None:
        if self.get_temp_num() == 0:
            self.show_error(Operator.ERROR_UNDEFINED)
        else:
            self.show_error(Operator.ERROR_ZERO_DIV)
    ...
    def remove_error(self) -> None:
        if self.entry.text() in (Operator.ERROR_UNDEFINED, Operator.ERROR_ZERO_DIV):
            self.entry.setMaxLength(self.entry_max_len)
            self.entry.setText('0')
            self.adjust_entry_font_size()
    ...
    def disable_buttons(self, disable: bool) -> None:
        for btn in Operator.BUTTONS_TO_DISABLE:
    ...
    def adjust_entry_font_size(self) -> None:
        font_size = Operator.DEFAULT_ENTRY_FONT_SIZE
        ...
        while self.get_entry_text_width() < self.entry.width() - 60:
            ...
            if font_size > Operator.DEFAULT_ENTRY_FONT_SIZE:
                break

            self.entry.setStyleSheet(f'font-size: {font_size}pt; border: none;')

    def adjust_temp_font_size(self) -> None:
        font_size = Operator.DEFAULT_FONT_SIZE
        ...
        while self.get_temp_text_width() < self.temp.width() - 60:
            ...
            if font_size > Operator.DEFAULT_FONT_SIZE:
                break
